...
6,8,9,10,6,7,4,3] #this is the list
count={} # this is the dictionary 
for item in elem:
    if item in count:
        count[item]+=1 #checks if number is in dictionary and stores new value 
    else:
        count[item]=1 #inserts item in dictionary 
sorted_count=sorted(count.items()) #sorting dictionary in key ascending order 
for i in sorted_count: # so it prints vertical
    print(i[0],':', i[1])
...
plt.bar(x,y)
plt.show()
y=json.dumps(sorted_count) # changing it into jason under thr variable y
print (y)
with open('y.jason', 'w') as out: #w is for write 
    json.dump(count, out)
...
plt.bar(list(sum_month.index.values),sum_month, width = 0.99)
plt.show()
